...
class Play:
    def __init__(self, xmlfile: str) -> None:
        self._source_file:str = xmlfile
        self._text_array = []
        self._xml_declarations = ["<?xml version='1.0' encoding='UTF-8'?>",
                                  "<?xml-model href='../../schemas/perseus_drama.rng' type='application/xml' schematypens='http://relaxng.org/ns/structure/1.0'?>"]


    def parse_source_file(self) -> None:
        start:bool = False
            ...
            for line in f:
                ...
                if start is False:
                    if re.match(r'^.*<TEI.2>', line):
                        start = True
                        for decl in self._xml_declarations:
                            self._text_array.append(decl)
                        self._text_array.append("<TEI xmlns=\"http://www.tei-c.org/ns/1.0\">")
                elif re.match(r'^.*</TEI.2>', line):
                    self._text_array.append("</TEI>")
                else:
                    self._text_array.append(self.convert_entities(line))
    ...
    def convert_entities(self, line) -> str:
...
def run():
    sourcedir = Path("/Users/wulfmanc/repos/gh/PerseusDL/canonical-engLit/Renaissance/Shakespeare/opensource")

    targetdir = Path("/Users/wulfmanc/repos/gh/PerseusDL/canonical-engLit/data/working")

    for path in sourcedir.glob("*.xml"):
        play = Play(path)
        play.parse_source_file()
        play.write(targetdir / Path(path.name))
